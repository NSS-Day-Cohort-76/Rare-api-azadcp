...
def list_post(query_params=None):
    with sqlite3.connect("./db.sqlite3") as conn:
        ...
        # 🔍 Check for special query: posts from subscriptions (home feed)
        if query_params and query_params.get("subscribedTo"):
            follower_id = int(query_params["subscribedTo"][0])
            db_cursor.execute(
                """
                SELECT
                    p.id,
                ...
                WHERE p.user_id IN (
                    SELECT author_id
                    FROM Subscriptions
                    WHERE follower_id = ? AND ended_on IS NULL
                )
                """,
                (follower_id,),
            )
